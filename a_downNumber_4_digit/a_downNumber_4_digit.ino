...
#define buttonPin A0
#define buzzerPin A2
int targetNumber;       //시리얼 모니터에서 입력받는 숫자
int currentNumber;      //현재 세그먼트 디스플레이에 표시되는 숫자
bool isPaused = false;      //숫자 증가의 일시중지 여부를 판단하는 bool 변수
unsigned long timer;        //경과시간 저장 변수

void setup() {
  byte numDigits = 4;                           //두자리만 사용(십의자리, 일의자리)
  byte digitPins[] = {2,5,6,13};                   // 10의 자리에 해당되는 공통단자 핀번호 6, 1의 자리에 해당되는 공통단자 핀번호 13
...
void loop() {
  if (Serial.available() > 0) {                 //시리얼버퍼에 값(데이터)이 들어오면 if문 아래 실행
    ...
    if (inputValue >= 1 && inputValue <= 9999) {  //inputValue는 1~99사이 정수
             //설정된 inputValue를 tagetNumber로 재설정
      currentNumber = inputValue;                        //현재 숫자 0으로 초기화
      targetNumber = 0;     
      isPaused = false;                         //숫자 증가의 일시중지 X (처음 시작할때는 증가해야하니까 false)
    }
  }

  if (digitalRead(buttonPin) == LOW) {          //버튼이 눌렸다면
...
//일시중지 상태가 아니고 && 현재 숫자 < 입력한 숫자 && 경과 시간이 0.3초일때
  if (!isPaused && currentNumber > targetNumber && millis() - timer >= 10) {
