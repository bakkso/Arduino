...
#define TI 480
#define DELAY 300
int num = 0;							   // buz on&off 조절 변수
int level = 0;							   // 음계 레벨 조절 변수
int c = 200;							   // 박자 초기값
...
void loop()
{
	...
	{ // 데이터 존재 여부 확인
		...
		int val = Serial.parseInt(); // 입력받은 값중 "정수값"만 읽어와서 val 변수에 저장

		if (val == 1 && num == 0)
		{ // num == 0을 받는 이유 : 1을 다시 입력했을때 종료조건을 만들어주기위해
		...
		if (num == 1)
		{ // 마지막 조건인 "1을 제외한 동작은 부저가 "ON" 상태일때만 작동한다를 만족시켜주기 위해
			...
			{
				if (c <= 800)
				{			   // 박자 = 출력시간, 즉 출력시간을 조절 시켜주면 됨. 조건 : 최댓값 800으로 설정
